async function loadDashboard() {
 ...
 <div class="two-col"><section class="panel"><h3>AI Slot Detection (Live)</h3><div id="dashSlots" class="slot-grid"></div></section><section class="panel"><h3>AI Recommendation</h3><div id="dashAI" class="recommendation">Calculating...</div><h3>Recent Vehicles</h3><div id="recentVehicles"></div></section></div></main>`;
    try {
        const r = await fetch(API_BASE + '/parking-slots');
        const slots = await r.json();
        const av = slots.filter(s => String(s.status).toUpperCase() === 'AVAILABLE'),
            oc = slots.length - av.length;
        ...
        const best = av.sort((a, b) => a.pricePerHour - b.pricePerHour)[0];
        document.getElementById('dashAI').innerHTML = best ? `<b>Best slot: ${best.slotNumber}</b><br>₹${best.pricePerHour}/hour • ${best.vehicleType}<br><small>Recommended by current availability and price.</small>` : 'No available slot.';
